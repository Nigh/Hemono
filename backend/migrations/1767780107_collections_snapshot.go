	...
	m.Register(func(app core.App) error {
		jsonData := `[
			...
			},
			{
				...
				"id": "pbc_1212822754",
				"indexes": [],
				"listRule": "owner = @request.auth.id || ledger_members_via_ledger.user ?= @request.auth.id",
				"name": "ledgers",
				"system": false,
				"type": "base",
			...
			{
				"createRule": "ledger.owner = @request.auth.id || ledger.ledger_members_via_ledger.user ?= @request.auth.id",
				"deleteRule": "payer = @request.auth.id",
				"fields": [
					{
						"autogeneratePattern": "[a-z0-9]{15}",
						"hidden": false,
